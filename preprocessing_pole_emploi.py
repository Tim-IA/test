...
df_adaptif["typeContrat"]=np.where(df_adaptif["natureContrat"].str.contains('apprentissage'), 'apprentissage', df_adaptif["typeContrat"])
df_adaptif["typeContrat"]=np.where(df_adaptif["natureContrat"].str.contains('professionnalisation'), 'professionnalisation', df_adaptif["typeContrat"])
...
df_adaptif['Stage']=np.where(df_adaptif[['intitule']].apply(lambda x: x.str.contains('stage')).any(1), 'stage', None)

df_adaptif['Stage'] = np.where(df_adaptif['Average']> 20000, 'cdi', df_adaptif['Stage'])

df_adaptif['Apprentissage']=np.where(df_adaptif[['intitule', 'description']].apply(lambda x: x.str.contains('altenance/alternant', case=False, regex=True)).any(1), 'apprentissage', None)
df_adaptif['CDI']=np.where(df_adaptif[['intitule', 'description']].apply(lambda x: x.str.contains('cdi')).any(1), 'cdi', None)
df_adaptif['Freelance']=np.where(df_adaptif[['intitule']].apply(lambda x: x.str.contains('freelance')).any(1), 'freelance', None)
df_adaptif['Freelance'] = np.where(df_adaptif['Average']< 2000, 'freelance', df_adaptif['Freelance'])
                                                                                                                   
df_adaptif['CDD']=np.where(df_adaptif[['intitule', 'description']].apply(lambda x: x.str.contains('cdd')).any(1), 'cdd', None)

df_adaptif['Contrat'] = df_adaptif['Contrat'].fillna(df_adaptif['Apprentissage'])
df_adaptif['Contrat'] = df_adaptif['Contrat'].fillna(df_adaptif['CDD'])
df_adaptif['Contrat'] = df_adaptif['Contrat'].fillna(df_adaptif['Freelance'])
df_adaptif['Contrat'] = df_adaptif['Contrat'].fillna(df_adaptif['CDI'])
df_adaptif['Contrat'] = df_adaptif['Contrat'].fillna(df_adaptif['Stage'])
df_adaptif['Contrat'] = df_adaptif['Contrat'].fillna(df_adaptif['typeContrat'])                                              



df_adaptif['Statut']=np.where(df_adaptif[['Contrat']].apply(lambda x: x.str.contains('stage|freelance', 
                                                                                     case=False, regex=True)).any(1), 0, None)
df_adaptif['Statut'] = df_adaptif['Statut'].fillna(df_adaptif['qualificationCode'])
...
#df_adaptif = df_adaptif.replace({np.nan: None})
#df_adaptif['Average'] = pd.to_numeric(df_adaptif['Average'],errors='coerce')

#df_adaptif['salaire_moyen'].df_adaptif['salaire_moyen'].fillna(df_adaptif['Average'])

#df_adaptif['salaire_moyen']=df_adaptif['salaire_moyen']+df_adaptif['Average']
...
df_adaptif['dureeAlternance'] = np.where(df_adaptif[['Contrat']].apply(lambda x: x.str.contains('apprentissage|professionnalisation',
                                                 case=False, regex=True)).any(1), df_adaptif['dureeTravail']*34, None)

df_adaptif['dureeStage'] = np.where(df_adaptif[['Contrat']].apply(lambda x: x.str.contains('stage',
                                                 case=False, regex=True)).any(1),df_adaptif['dureeTravail']*22, None)


df_adaptif['dureeTravail_annelle'] = np.where(df_adaptif[['Contrat']].apply(lambda x: x.str.contains('freelance',
                                                 case=False, regex=True)).any(1), 7, None)
df_adaptif['dureeOrd']=df_adaptif['dureeTravail'] / 7*302
df_adaptif['dureeTravail_annelle']=df_adaptif['dureeTravail_annelle'].fillna(df_adaptif['dureeAlternance'])
df_adaptif['dureeTravail_annelle']=df_adaptif['dureeTravail_annelle'].fillna(df_adaptif['dureeStage'])
df_adaptif['dureeTravail_annelle']=df_adaptif['dureeTravail_annelle'].fillna(df_adaptif['dureeOrd'])
df_adaptif['dureeTravail_annelle']=df_adaptif['dureeTravail_annelle'].astype(int)
...
df_adaptif['experience'] = df_adaptif['experience'].astype(int)
...
df_adaptif=df_adaptif.drop(['salaire_min','salaire_max','Statut1', 'Statut2', 'CDD', 'plein','mexperience','cexperience', 
                           'salaire', 'Freelance', 'CDI','Stage','Statut3','dureeTravail1','Sexperience','dexperience',
                           'Apprentissage', 'dureeTravail2','bac+1','salaire_moyen', 'jexperience','1experience', 'experience1',
                           'dureeOrd', 'dureeAlternance', 'dureeStage', 'dureeTravail'],axis=1)
...
df_adaptif['autre bonus']=np.where(df_adaptif[['description']].apply(lambda x: x.str.contains('chèque repas|ticket|restaurant',
                                                 case=False, regex=True)).any(1), 1, df_adaptif['autre bonus'])

df_adaptif['Python'] = np.where(df_adaptif[['intitule', 'appellationlibelle', 'description',
                                            'entreprise.description', 'competences']].apply(lambda x: x.str.contains('python',
                                                 case=False, regex=True)).any(1), 1, 0)

df_adaptif['R/Scala/Matlab/SAS'] = np.where(df_adaptif[['intitule', 'appellationlibelle', 'description',
                                            'entreprise.description', 'competences']].apply(lambda x: x.str.contains('julia|rstudio|matlab|scala|sas',
                                                 case=False, regex=True)).any(1), 1, 0)
df_adaptif['hadoop'] = np.where(df_adaptif[['intitule', 'appellationlibelle', 'description',
                                            'entreprise.description', 'competences']].apply(lambda x: x.str.contains('hadoop',
                                                 case=False, regex=True)).any(1), 1, 0) 
df_adaptif['librairies_data'] = np.where(df_adaptif[['intitule', 'appellationlibelle', 'description',
                                            'entreprise.description', 'competences']].apply(lambda x: x.str.contains('databricks|spark|pandas|numpy|scikit-learn|tensorflow|keras|pytorch',
                                                                                                                     case=False, regex=True)).any(1), 1, 0)                                                                                                                    

df_adaptif['Kafka'] = np.where(df_adaptif[['intitule', 'appellationlibelle', 'description',
                                            'entreprise.description', 'competences']].apply(lambda x: x.str.contains('kafka ',
                                                 case=False, regex=True)).any(1), 1, 0)                                                                                                                   
...
df_adaptif['autres_langages'] = np.where(df_adaptif[['intitule', 'appellationlibelle', 'description',
                                            'entreprise.description', 'competences']].apply(lambda x: x.str.contains('java|flutter|.net',
                                                 case=False, regex=True)).any(1), 1, 0)
...
new_df = df_adaptif.filter(['poste','description', 'Departement', 
                            'experience','Contrat', 'Statut', 'dureeTravail_annelle',
                            'bac+', 'Anglais','mutuelle', 'autre bonus',  
                            'Python','R/Scala/Matlab/SAS','librairies_data', 
                            'langages_Web','autres_langages',
                            'bases_donnees','cloud', 'hadoop', 'app_analyse',
                            'Kafka', 'motscles','Average'], axis=1)                                                                                                                         
                                                                                                                     #df_adaptif.info()
